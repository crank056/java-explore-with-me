...
public class RequestService {
    ...
    public List<ParticipationRequestDto> getAllRequestFromUserEvent(Long userId, Long eventId)
        ...
        List<ParticipationRequestDto> listPRDto = new ArrayList<>();
        for (Request request : list) {
            listPRDto.add(RequestMapper.toPRDto(request));
        }
        return listPRDto;
    }
    ...
    public ParticipationRequestDto confirmRequest(Long userId, Long eventId, Long reqId)
        ...
        if (!requestRepository.existsById(reqId)) throw new NotFoundException("Запроса не существует");
    ...
    public ParticipationRequestDto createRequest(Long userId, Long eventId) throws AccessException, NotFoundException {
        if (!userRepository.existsById(userId)) throw new NotFoundException("Пользователя не существует");
        if (!eventRepository.existsById(eventId)) throw new NotFoundException("Мероприятия не существует");
        if (requestRepository.findByRequesterIdAndEventId(userId, eventId) != null) throw new AccessException(
            "Нельзя дважды добавить запрос");
        Event event = eventRepository.getReferenceById(eventId);
        User user = userRepository.getReferenceById(userId);
        if (event.getInitiator().getId().equals(userId)) throw new AccessException(
            "Нельзя добавить запрос на участие в своем мероприятии");
        if (!event.getState().equals(State.PUBLISHED)) throw new AccessException("Мероприятие не опубликовано");
        if (event.getParticipantLimit() <= event.getConfirmedRequests()) throw new AccessException(
